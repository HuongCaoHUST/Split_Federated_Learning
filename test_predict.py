...
# 1. Khởi tạo màu sắc ngẫu nhiên cố định cho mỗi class (tối đa 80 class)
# Mỗi lần chạy lại code màu sẽ giống nhau nhờ vào random.seed
random.seed(42)
...
for i, det in enumerate(preds):
    if len(det):
        det[:, :4] = ops.scale_boxes(img.shape[2:], det[:, :4], ori_img.shape).round()
        ...
        for *xyxy, conf, cls in det:
            ...
            cv2.rectangle(ori_img, c1, c2, color, -1, cv2.LINE_AA)  
